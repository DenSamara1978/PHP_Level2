...
<main>
    ...
    <?php 
        $frame = isset ( $_GET [ 'frame' ] ) ? $_GET ['frame' ] : 1;
        $prevFrame = ( $frame > 1 ) ? ( $frame - 1 ) : $frame;
        $nextFrame = $frame + 1;

        $prevString = 'index.php?frame=' . $prevFrame;
        $nextString = 'index.php?frame=' . $nextFrame;
    ?>

    <div class='browsePanel'>
        <input type='button' class='browse btn' value='Предыдущие товары' onClick="javascript:document.location.href='<?= $prevString ?>'">
        <input type='button' class='browse btn' value='Следующие товары' onClick="javascript:document.location.href='<?= $nextString ?>'">
    </div>
